...
/* harmony import */ var buffer___WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! buffer/ */ "../common/node_modules/buffer/index.js");
...
function requireCodecs () {
	...
	(function (exports) {
		...
		function serializeFrameWithLength(frame) {
		    ...
		    var lengthPrefixed = buffer___WEBPACK_IMPORTED_MODULE_2__.Buffer.allocUnsafe(buffer.length + UINT24_SIZE);
		...
		function serializeSetupFrame(frame) {
		    ...
		    var metadataMimeTypeLength = frame.metadataMimeType != null
		        ? buffer___WEBPACK_IMPORTED_MODULE_2__.Buffer.byteLength(frame.metadataMimeType, "ascii")
		        : 0;
		    var dataMimeTypeLength = frame.dataMimeType != null
		        ? buffer___WEBPACK_IMPORTED_MODULE_2__.Buffer.byteLength(frame.dataMimeType, "ascii")
		        : 0;
		    var payloadLength = getPayloadLength(frame);
		    var buffer = buffer___WEBPACK_IMPORTED_MODULE_2__.Buffer.allocUnsafe(FRAME_HEADER_SIZE +
		        SETUP_FIXED_SIZE + //
		...
		function serializeErrorFrame(frame) {
		    var messageLength = frame.message != null ? buffer___WEBPACK_IMPORTED_MODULE_2__.Buffer.byteLength(frame.message, "utf8") : 0;
		    var buffer = buffer___WEBPACK_IMPORTED_MODULE_2__.Buffer.allocUnsafe(FRAME_HEADER_SIZE + ERROR_FIXED_SIZE + messageLength);
		    var offset = writeHeader(frame, buffer);
		...
		function serializeKeepAliveFrame(frame) {
		    ...
		    var buffer = buffer___WEBPACK_IMPORTED_MODULE_2__.Buffer.allocUnsafe(FRAME_HEADER_SIZE + KEEPALIVE_FIXED_SIZE + dataLength);
		    var offset = writeHeader(frame, buffer);
		...
		function serializeLeaseFrame(frame) {
		    ...
		    var buffer = buffer___WEBPACK_IMPORTED_MODULE_2__.Buffer.allocUnsafe(FRAME_HEADER_SIZE + LEASE_FIXED_SIZE + metaLength);
		    var offset = writeHeader(frame, buffer);
		...
		function serializeRequestFrame(frame) {
		    var payloadLength = getPayloadLength(frame);
		    var buffer = buffer___WEBPACK_IMPORTED_MODULE_2__.Buffer.allocUnsafe(FRAME_HEADER_SIZE + payloadLength);
		    var offset = writeHeader(frame, buffer);
		    writePayload(frame, buffer, offset);
		    return buffer;
		}
		...
		function serializeMetadataPushFrame(frame) {
		    ...
		    if (metadata != null) {
		        var buffer = buffer___WEBPACK_IMPORTED_MODULE_2__.Buffer.allocUnsafe(FRAME_HEADER_SIZE + metadata.byteLength);
		        var offset = writeHeader(frame, buffer);
		...
		function serializeRequestManyFrame(frame) {
		    var payloadLength = getPayloadLength(frame);
		    var buffer = buffer___WEBPACK_IMPORTED_MODULE_2__.Buffer.allocUnsafe(FRAME_HEADER_SIZE + REQUEST_MANY_HEADER + payloadLength);
		    var offset = writeHeader(frame, buffer);
		...
		function serializeRequestNFrame(frame) {
		    var buffer = buffer___WEBPACK_IMPORTED_MODULE_2__.Buffer.allocUnsafe(FRAME_HEADER_SIZE + REQUEST_N_HEADER);
		    var offset = writeHeader(frame, buffer);
		...
		function serializeResumeFrame(frame) {
		    ...
		    var buffer = buffer___WEBPACK_IMPORTED_MODULE_2__.Buffer.allocUnsafe(FRAME_HEADER_SIZE + RESUME_FIXED_SIZE + resumeTokenLength);
		    var offset = writeHeader(frame, buffer);
		    offset = buffer.writeUInt16BE(frame.majorVersion, offset);
		    offset = buffer.writeUInt16BE(frame.minorVersion, offset);
		    offset = buffer.writeUInt16BE(resumeTokenLength, offset);
		...
		function serializeResumeOkFrame(frame) {
		    var buffer = buffer___WEBPACK_IMPORTED_MODULE_2__.Buffer.allocUnsafe(FRAME_HEADER_SIZE + RESUME_OK_FIXED_SIZE);
		    var offset = writeHeader(frame, buffer);
		    writeUInt64BE(buffer, frame.clientPosition, offset);
		    return buffer;
		}
	    ...
	    return __generator(this, function (_c) {
	        switch (_c.label) {
	            case 0:
	                ...
	                metadata = buffer___WEBPACK_IMPORTED_MODULE_2__.Buffer.allocUnsafe(0);
	                return [3 /*break*/, 6];
	            case 1:
	                metadataPosition = 0;
...
function requireReassembler () {
	...
	function add(holder, dataFragment, metadataFragment) {
	    ...
	    holder.data = holder.data
	        ? buffer___WEBPACK_IMPORTED_MODULE_2__.Buffer.concat([holder.data, dataFragment])
	        : dataFragment;
	    if (holder.metadata && metadataFragment) {
	        holder.metadata = buffer___WEBPACK_IMPORTED_MODULE_2__.Buffer.concat([holder.metadata, metadataFragment]);
	    }
	    return true;
	}
	Reassembler.add = add;
	function reassemble(holder, dataFragment, metadataFragment) {
	    ...
	    if (holder.metadata) {
	        var metadata = metadataFragment
	            ? buffer___WEBPACK_IMPORTED_MODULE_2__.Buffer.concat([holder.metadata, metadataFragment])
	            : holder.metadata;
	...
	var WebsocketDuplexConnection$1 = /** @class */ (function (_super) {
	    ...
	    function WebsocketDuplexConnection(websocket, deserializer, multiplexerDemultiplexerFactory) {
	        ...
	        _this.handleMessage = function (message) {
	            try {
	                var buffer = buffer___WEBPACK_IMPORTED_MODULE_2__.Buffer.from(message.data);
...
class AbstractRemote {
    ...
    async socketStreamRaw(options, map, bson) {
        ...
        function toBuffer(js) {
            ...
            return buffer___WEBPACK_IMPORTED_MODULE_2__.Buffer.from(contents);
        }
...
SELECT * FROM crud_entries;
    ...
    watchWithAsyncGenerator(sql, parameters, options) {
        return new event_iterator__WEBPACK_IMPORTED_MODULE_1__.EventIterator((eventOptions) => {
            const handler = {
                onResult: (result) => {
...
/***/ "../common/node_modules/buffer/index.js":
/*!**********************************************!*\
  !*** ../common/node_modules/buffer/index.js ***!
  \**********************************************/
/***/ ((__unused_webpack_module, exports, __webpack_require__) => {
