...
export default function EditGatewayScreen() {
  ...
  const { data: gatewayResults = [] } = useQuery<Gateway>(
    `SELECT * FROM ${gatewaysTable} WHERE id = ?`,
    [gatewayId]
  );
  const gateway = gatewayResults?.[0] || null;
  const gatewayLoading = !gateway && gatewayResults.length === 0;

  const siteIdForQueries = formData.site_id || gateway?.site_id;
  ...
  const updateMutation = useMutation({
    mutationFn: (data: Partial<Gateway>) => gatewayService.updateGateway(gatewayId!, data),
    onSuccess: () => {
    ...
    onError: (error: any) => {
      Alert.alert('Error', error.message || 'Failed to update gateway');
    },
  });
